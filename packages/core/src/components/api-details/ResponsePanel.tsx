...
function getStatusDotColor(status: number): string {
    if (status >= 200 && status < 300) return 'bg-green-500 ring-2 ring-green-500/30';
    if (status >= 400) return 'bg-destructive ring-2 ring-destructive/30';
        ...
        return (
            <div className="flex flex-col h-full bg-muted/30">
                ...
                <div className="h-12 border-b border-border flex items-center px-4 shrink-0">
                    <span className="font-bold text-sm text-muted-foreground uppercase tracking-wider">Response</span>
                </div>
                <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground gap-3 p-8 cursor-default">
                    <Send className="w-8 h-8 opacity-20"/>
                    <span className="text-sm font-medium">Send a request to see the response</span>
                </div>
            </div>
        );
    }
        ...
        <div className="flex flex-col h-full bg-muted/30">
            ...
            <div className="h-12 border-b border-border flex items-center justify-between px-4 shrink-0">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-muted-foreground">{response!.time}ms</span>
                    <div className="h-3 w-px bg-border"/>
                    <span className="text-sm font-bold text-muted-foreground">{response!.size}KB</span>
                </div>
            </div>
            ...
            <div className="flex items-center px-4 border-b border-border gap-4 shrink-0">
                ...
                >
                    ...
                    {headerEntries.length > 0 && (
                        <span className="ml-1 text-xs bg-muted px-1.5 rounded text-muted-foreground font-bold">
            ...
            {activeTab === 'body' ? (
                ...
                <div className="flex-1 overflow-y-auto p-4 min-h-[300px] md:min-h-0" data-testid="response-headers-content">
                    {headerEntries.length > 0 ? (
                        <div className="space-y-3">
                            <div className="flex justify-end">
                                <button
                                    ...
                                    className="text-sm font-bold text-muted-foreground hover:text-foreground cursor-pointer flex items-center gap-1.5 transition-colors uppercase tracking-tight"
                                >
                            ...
                            <div className="border border-border rounded-md overflow-hidden">
                                <table className="w-full text-sm">
                                    ...
                                    <tr>
                                        <th className="p-3 text-left font-bold text-foreground w-1/3 uppercase tracking-wider text-xs">Header</th>
                                        <th className="p-3 text-left font-bold text-foreground uppercase tracking-wider text-xs">Value</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    {headerEntries.map(([key, value]) => (
                                        <tr key={key} className="border-t border-border">
                                            <td className="p-3 font-mono text-foreground font-bold">{key}</td>
                                            <td className="p-3 font-mono text-muted-foreground break-all">{value}</td>
                                        </tr>
                                    ))}
                                    </tbody>
                                </table>
                        ...
                        <div className="text-sm text-muted-foreground font-medium italic">No response headers</div>
                    )}
                </div>
            )}
        </div>
