...
export function EndpointSidebar({
  ...
  const endpointsRef = useRef<(HTMLButtonElement | null)[]>([]);
  ...
  // Stagger animate endpoints when expanded groups or search change
  useEffect(() => {
    const validRefs = endpointsRef.current.filter((ref): ref is HTMLButtonElement => ref !== null);
    if (validRefs.length > 0) {
      staggerFadeIn(validRefs, 15);
    }
  }, [expandedGroups, searchQuery]);
    ...
    return (
      <div className={`flex flex-col ${className}`}>
        <div className="p-4 border-b border-border">
          <h3 className="text-base font-bold text-card-foreground mb-1">Endpoints</h3>
          <p className="text-sm text-muted-foreground">{endpoints.length} operations</p>
        </div>
          ...
          {Object.entries(spec.paths).map(([path, pathItem]: [string, any]) => {
            return Object.entries(pathItem as PathItemObject)
              ...
              .map(([method, operation]: [string, any]) => {
                ...
                return (
                  <button
                    ref={el => { endpointsRef.current[currentIndex] = el; }}
                    key={`${method}-${path}`}
                  ...
                  >
                    ...
                    <div
                      className={`text-sm font-mono mb-1 transition-colors ${
                        isSelected
  ...
  return (
    <div className={`flex flex-col bg-muted/30 ${className}`}>
      ...
      <div className="p-3 border-b border-border shrink-0">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-background border border-border text-foreground text-sm rounded pl-8 pr-3 h-9 focus:outline-none focus:border-primary transition-all placeholder:text-muted-foreground cursor-text"
          />
        </div>
      </div>
          ...
          filteredGroups.map((group) => (
            <div key={group.tag} className="mb-2">
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 hover:bg-muted/50 text-sm font-bold text-foreground cursor-pointer transition-colors uppercase tracking-wider"
              >
                  ...
                  {group.endpoints.map((endpoint) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-muted/50 cursor-pointer transition-colors ${
                          isSelected ? 'bg-muted font-bold' : 'font-medium'
                        }`}
                      >
                        <span
                          className={`font-bold w-12 text-xs md:text-sm ${getMethodColor(endpoint.method).text}`}
                        >
                          {endpoint.method}
                        </span>
      ...
      {/* Footer */}
      <div className="p-3 border-t border-border text-xs md:text-sm font-medium text-muted-foreground shrink-0 cursor-default">
