...
describe('generateExampleFromSchema - requestBody $ref resolution', () => {
    /**
     * Regression test: when requestBody is a $ref (e.g. `{ $ref: '#/components/requestBodies/Foo' }`),
     * callers must resolve the ref before accessing .content — otherwise .content is undefined
     * and the schema never populates in Try It Out.
     */
    const spec = {
        components: {
            schemas: {
    ...
    it('resolves requestBody $ref to access its content', () => {
        const requestBodyRef = {$ref: '#/components/requestBodies/CreatePetBody'};
        const resolved = resolveRef(requestBodyRef.$ref, spec) as any;
        expect(resolved).not.toBeNull();
        const jsonContent = resolved?.content?.['application/json'];
        expect(jsonContent).toBeDefined();
        const result = JSON.parse(generateExampleFromSchema(jsonContent.schema, 0, spec));
        expect(result).toEqual({name: 'Buddy', species: 'dog'});
    });

    it('generates example from inline requestBody schema via $ref chain', () => {
        // When the schema inside the requestBody is itself a $ref, it should resolve fully
        const schema = {$ref: '#/components/schemas/CreatePet'};
        const result = JSON.parse(generateExampleFromSchema(schema, 0, spec));
        expect(result).toEqual({name: 'Buddy', species: 'dog'});
    });
});
