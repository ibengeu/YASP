...
import {AlertCircle, ArrowLeft, Loader2, Server, Upload} from 'lucide-react';
...
import {EndpointGroup, EndpointInfo, OpenAPISpec} from './types';
                ...
                >
                    <Card className="border-border/50 card-shadow-lg bg-card/50 backdrop-blur-sm">
                        <CardContent className="p-8 text-center space-y-6">
                            <div
                                className="h-16 w-16 rounded-full bg-primary/10 flex items-center justify-center mx-auto">
                                <Upload className="h-6 w-6 text-primary"/>
                            </div>
                            <div className="space-y-2">
                                <h2 className="text-xl font-semibold tracking-tight">Load API Specification</h2>
                                <p className="text-muted-foreground leading-relaxed">
                                    Upload an OpenAPI 3.0 JSON file to explore your API, or use our demo API to get
                                    started.
                                </p>
                            </div>
                            <div className="space-y-3">
                                ...
                                <Button onClick={() => setSpec(demoApiSpec)}
    ...
    return (
        <div className="h-full flex flex-col bg-background">
            ...
            <motion.div
                ...
                animate={{opacity: 1, y: 0}}
                className="border-b border-border/50 bg-background/95 backdrop-blur-sm shrink-0"
            >
                <div className="space-y-4">
                    {/* Back to Catalog - Full width at top */}
                    {onBackToCatalog && (
                        <div className="px-6 pt-6">
                            <Button
                                ...
                                className="hover:bg-secondary/50 -ml-3"
                            >
                            ...
                            {spec.servers && spec.servers.length > 0 && (
                                <div className="flex items-center gap-3">
                                    ...
                                    <Select value={selectedServer} onValueChange={setSelectedServer}>
                                        <SelectTrigger
                                            className="w-64 bg-input-background border-border/50 focus:border-primary focus:ring-primary/30 text-sm">
                                            <SelectValue/>
                                        </SelectTrigger>
                                        <SelectContent className="border-border/50 card-shadow">
                                            {spec.servers.map((server, index) => (
                                                <SelectItem key={index} value={server.url}>
                                                    <div className="flex flex-col items-start">
                                                        ...
                                                        {server.description && (
                                                            <span
                                                                className="text-muted-foreground">{server.description}</span>
                                                        )}
                                                    </div>
                                                </SelectItem>
                                            ))}
            ...
            {/* Main content - three resizable columns */}
            <div className="flex-1 min-h-0">
                <ResizablePanelGroup direction="horizontal" className="h-full">
                    {/* Column 1: Endpoints List */}
                    <ResizablePanel defaultSize={25} minSize={20} maxSize={40} className="bg-sidebar/30">
                    ...
                    {/* Column 3: Try It Out - Full height with distinct background */}
                    <ResizablePanel defaultSize={30} minSize={25} maxSize={50} className="bg-sidebar/20">
