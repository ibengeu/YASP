/**
 * OpenAPI Specification Inference Utility
 *
 * Automatically extracts metadata from OpenAPI 3.x specifications
...
/**
 * Main inference function - extracts all available metadata from OpenAPI spec
 *
 * @param spec - Parsed OpenAPI 3.x specification object
...
export function inferAllData(spec: any, sourceUrl?: string): InferredData {
  ...
  const servers = extractServers(spec.servers, validationIssues, sourceUrl);
  const primaryServerUrl = servers.find(s => s.isDefault)?.url || servers[0]?.url;

  // Extract authentication
  const { auth, multipleAuthSupported } = extractAuth(spec);

  // Extract tags
  const tags = extractTags(spec.tags);

  // Count endpoints
  const { endpointCount, endpointsByMethod } = countEndpoints(spec.paths);
...
/**
 * Count endpoints and group by HTTP method
 */
