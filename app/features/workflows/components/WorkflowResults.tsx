              ...
              <AccordionContent className="px-3 pb-3">
                ...
                {result.response && (
                  <div>
                    ...
                    <pre className="text-[10px] font-mono text-foreground bg-muted rounded-md p-2 max-h-40 overflow-auto">
                ...
                {Object.keys(result.extractedVariables).length > 0 && (
                  <div className="mt-2">
                    <span className="text-[10px] text-muted-foreground font-medium">Extracted:</span>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {Object.entries(result.extractedVariables).map(([name, value]) => (
                        <span
                          key={name}
                          className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-mono bg-blue-500/10 text-blue-600 dark:text-blue-400"
                        >
